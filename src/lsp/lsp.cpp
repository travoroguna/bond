    ...
    void langserver_handler::next() {
        ...
        if (msg.is_request()) {
            ...
            if (req.method() == "initialize") {
                ...
                auto params = completion_params::from_json(req.params());
                auto completion_list = m_Langserver->on_completion(params);
                auto response = req.reply(completion_list.to_json());
                m_Connection.write(response);
            } else if(req.method() == "textDocument/diagnostic") {
                auto params = document_diagnostic_params::from_json(req.params());
                auto diagnostic_list = m_Langserver->on_diagnostic(params);
                auto response = req.reply(diagnostic_list.to_json());
                m_Connection.write(response);
            }
            ...
            } else if(noti.method() == "$/cancelRequest"){
                auto id = noti.params()["id"];
                auto resp = rpc::response(id, (json)nullptr, rpc::response_error<json>(-32800, "Method not found", json{}));
//                std::cerr << "Canceling request " << id << " " << resp.to_json().dump() << std::endl;
                m_Connection.write(resp);

            } else if (noti.method() == "$/setTrace") {
                // do nothingv for now
...
// WorkspaceClientCapabilities
    ...
    workspace_client_capabilities::workspace_edit_t::from_json(json const &js) {
        ...
        return workspace_edit_t()
                ...
                .resource_operations(def(jw.opt("resourceOperations") | list(to_resource_operation_kind),
                                         std::vector<resource_operation_kind>()))
                .failure_handling(
                        def(jw.opt("failureHandling") | to_failure_handling_kind, failure_handling_kind::abort));
    }
    ...
    workspace_client_capabilities::execute_command_t
    workspace_client_capabilities::execute_command_t::from_json(json const &js) {
        auto jw = jwrap(js);
...
// TextDocumentClientCapabilities
    ...
    text_document_client_capabilities::completion_t::completion_item_t::from_json(json const &js) {
        ...
        return completion_item_t()
                ...
                .documentation_format(
                        def(jw.opt("documentationFormat") | list(to_markup_kind), std::vector<markup_kind>()))
    ...
    text_document_client_capabilities::document_symbol_t
    text_document_client_capabilities::document_symbol_t::from_json(json const &js) {
        auto jw = jwrap(js);
    ...
    text_document_client_capabilities::code_action_t::from_json(json const &js) {
        ...
        return code_action_t()
                ...
                .code_action_literal_support(
                        jw.opt("codeActionLiteralSupport") | code_action_literal_support_t::from_json);
    }
...
// ServerCapabilities
    ...
    json server_capabilities::to_json() const {
        return jbuild()
                ...
                .opt("diagnosticProvider", diagnostic_provider() | lift(any_to_json))
...
// FoldingRangeParams
    ...
    completion_params completion_params::from_json(json const &js) {
        auto jw = jwrap(js);

        return completion_params()
                .doc_context(context::from_json(jw.get("context")))
                .doc_position(position::from_json(jw.get("position")))
                .text_document(text_document_identifier::from_json(jw.get("textDocument")));
    }

    context context::from_json(const json &js) {
        auto jw = jwrap(js);
        return context().trigger_kind(jw.get("triggerKind"));
    }
...
// PublishDiagnosticsParams
    ...
    json completion_item::to_json() const {
        return jbuild()
                .set("label", label())
                .opt("kind", kind() | lift(any_to_json))
                .opt("detail", detail())
                .opt("documentation", documentation())
                .opt("insertText", insertText())
                .opt("sortText", sortText())
                .get();
    }

    json completion_list::to_json() const {
        return jbuild()
                .set("isIncomplete", is_incomplete())
                .set("items", vector_to_json(items()))
                .get();
    }

    json diagnostic_options::to_json() const {
        return jbuild()
                .set("identifier", identifier())
                .set("interFileDependencies", interFileDependencies())
                .set("workspaceDiagnostic", workspaceDiagnostic())
                .get();
    }

    document_diagnostic_params document_diagnostic_params::from_json(const json &js) {
        auto jw = jwrap(js);
        return document_diagnostic_params()
                .text_document(text_document_identifier::from_json(jw.get("textDocument")))
                .identifier(jw.opt("identifier"))
                .previous_result_id(jw.opt("previousResultId"));
    }
