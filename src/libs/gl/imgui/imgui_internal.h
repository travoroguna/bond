...
// [SECTION] ImGuiContext (main gl context)
...
// In 1.89.4, we moved the implementation of "courtesy maths operators" from imgui_internal.h in gl.h
...
#error Please '#define IMGUI_DEFINE_MATH_OPERATORS' _BEFORE_ including gl.h!
#endif
...
//-----------------------------------------------------------------------------
// [SECTION] Context pointer
// See implementation of this variable in gl.cpp for comments and details.
//-----------------------------------------------------------------------------
...
// This is going to be exposed in gl.h when stabilized enough.
enum ImGuiItemFlags_ {
    ...
    ImGuiItemFlags_Disabled = 1
            << 2,  // false     // Disable interactions but doesn't affect visuals. See BeginDisabled()/EndDisabled(). See github.com/ocornut/gl/issues/211
...
// Windows data saved in gl.ini file
...
struct IMGUI_API ImGuiWindow {
    ...
    short BeginOrderWithinContext;            // Begin() order within entire gl context. This is mostly used for debugging submission order related issues.
...
namespace ImGui {
    ...
    IMGUI_API ImDrawList *GetBackgroundDrawList(
            ImGuiViewport *viewport);                     // get background draw list for the given viewport. this draw list will be the first rendering one. Useful to quickly draw shapes/text behind dear gl contents.
    IMGUI_API ImDrawList *GetForegroundDrawList(
            ImGuiViewport *viewport);                     // get foreground draw list for the given viewport. this draw list will be the last rendered one. Useful to quickly draw shapes/text over dear gl contents.
